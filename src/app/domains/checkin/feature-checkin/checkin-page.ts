...
export class CheckinPage {
  ...
  // For lazy injection to work, the service being injected must be auto-provided (@service())
  private readonly upgradeService = injectAsync(
    () => import('./upgrade-service'),
    {
      prefetch: () => onIdle() // Fetch the bundle once the browser is idle
    }
  );
  ...
  protected readonly address = new SignalFormControl(this.addressFormModel(), (path) => {
    required(path.street);
    required(path.zipCode);
    required(path.country);
  });

  protected readonly phoneNumber = new SignalFormControl('1234 5678', (path) => {
    required(path);
  });
